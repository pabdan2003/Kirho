...
Motor analógico, motor digital y coordinador de señal mixta. Para la visión
arquitectónica completa ver
...
| `mixed_signal.py` | `MixedSignalInterface` — orquesta MNA + digital + puentes por ventanas de tiempo. `TimingAnalyzer` analiza setup/hold post-simulación. | Co-simular circuitos mixtos, analizar timing. |
...
1. **Cada modelo nuevo debe incluir un caso de prueba cerrado.** Sin una
   referencia (ej: `V_out = V_in · R2/(R1+R2)`) no hay forma de detectar
   regresiones en el motor.
