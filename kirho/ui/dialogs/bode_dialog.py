...
class BodeDialog(QDialog):
    ...
    def __init__(self, scene, colors, parent=None):
        ...
        self.scene = scene
        self.colors = colors
        self.setWindowTitle(self.tr('Bode / Transfer Function Analysis'))
    ...
    def _build_ui(self):
        ...
        gb_signal = QGroupBox(self.tr('Input / Output Signal'))
        ...
        form_signal.addRow(self.tr('AC source:'), self.cb_input)
        self.cb_out = QComboBox()
        form_signal.addRow(self.tr('Output node (V_out):'), self.cb_out)
        self.cb_ref = QComboBox()
        form_signal.addRow(self.tr('Reference node:'), self.cb_ref)
        left.addWidget(gb_signal)

        gb_sweep = QGroupBox(self.tr('Sweep'))
        ...
        form_sweep.addRow(self.tr('Start f:'), self.sb_fstart)
        ...
        form_sweep.addRow(self.tr('Stop f:'), self.sb_fstop)

        self.sp_points = QSpinBox()
        self.sp_points.setRange(10, 5000)
        self.sp_points.setValue(400)
        form_sweep.addRow(self.tr('Points:'), self.sp_points)

        self.cb_scale = QComboBox()
        self.cb_scale.addItem(self.tr('Logarithmic'), 'log')
        self.cb_scale.addItem(self.tr('Linear'), 'linear')
        form_sweep.addRow(self.tr('f scale:'), self.cb_scale)

        left.addWidget(gb_sweep)

        btn_row = QHBoxLayout()
        self.btn_run = QPushButton(self.tr('Calculate'))
        ...
        gb_cursor = QGroupBox(self.tr('Cursor (click the chart)'))
        form_c = QFormLayout(gb_cursor)
        self.lbl_freq = QLabel('—')
        self.lbl_mag  = QLabel('—')
        self.lbl_phase = QLabel('—')
        self.lbl_freq.setFont(QFont('Menlo', 10, QFont.Weight.Bold))
        self.lbl_mag.setFont(QFont('Menlo', 10, QFont.Weight.Bold))
        self.lbl_phase.setFont(QFont('Menlo', 10, QFont.Weight.Bold))
        form_c.addRow('f:', self.lbl_freq)
        form_c.addRow('|H|:', self.lbl_mag)
        form_c.addRow('∠H:', self.lbl_phase)
        left.addWidget(gb_cursor)

        gb_notable = QGroupBox(self.tr('Notable points'))
    ...
    def _setup_axes(self):
        ...
        self.ax_phase.set_xlabel(self.tr('Frequency (Hz)'))
        self.ax_mag.set_title(self.tr('Magnitude'))
        self.ax_phase.set_title(self.tr('Phase'))
    ...
    def _populate_sources_and_nodes(self):
        ...
        if not ac_items:
            QMessageBox.warning(
                self, self.tr('No AC sources'),
                self.tr('No VAC or FGEN sources were found in the circuit. '
    ...
    def _run_sweep(self):
        ...
        from kirho.ui.scene import build_engine_components_for_item
        from kirho.ui.items.component_item import ComponentItem
        from kirho.engine import MNASolver, VoltageSourceAC, Resistor
        ...
        if f_stop <= f_start:
            QMessageBox.warning(self, self.tr('Invalid range'),
                                self.tr('Stop f must be greater than start f.'))
            return
        points = int(self.sp_points.value())
        scale = self.cb_scale.currentData()
        ...
        if not comps:
            QMessageBox.warning(self, self.tr('Empty circuit'),
                                self.tr('There are no simulatable components.'))
            return
        ...
        if n_out not in all_nets:
            QMessageBox.warning(
                self, self.tr('Node not found'),
                self.tr('The output node "{node}" does not appear in the circuit.').format(node=n_out))
            return

        solver = MNASolver()
        res = solver.solve_ac(comps, f_start, f_stop, points=points, scale=scale)
        if not res.get('success'):
            QMessageBox.warning(self, self.tr('Sweep error'),
    ...
    def _update_notable_points(self, freqs: np.ndarray, H: np.ndarray):
        ...
        self.lbl_dc_gain.setText(
            self.tr('DC gain: {gain:+.3f} dB  ({ratio:.4g} V/V)').format(
                gain=g_dc_db, ratio=g_dc_lin))
        if fc is not None:
            self.lbl_fc_3db.setText(
                self.tr('f_cutoff (−3 dB): {frequency:.4g} Hz').format(frequency=fc))
        else:
            self.lbl_fc_3db.setText(self.tr('f_cutoff (−3 dB): no crossing in range'))
        self.lbl_slope.setText(
            self.tr('Final slope: {slope:+.1f} dB/dec').format(slope=slope))
